...
export default async function handler(req, res) {
    try {
        ...
        let customer = await FirebaseAdmin.firestore().collection("customers").where("email", "==", req.body["customer_email"]).get();
        ...
        const staff = await Staff.get(req.body["staff_id"]);
        // grab staff user account from Firebase
        const staffUserAccount = await FirebaseAdmin.auth().getUser(staff.uid);

        // get services from DB
        const services = await Promise.all(req.body["services"].map(async (doc_id) => {
        ...
        try {
            textMagicId = await sendSMSConfirmation(settings, new Date(postedEvent.start.dateTime), postedEvent.start.timeZone, staff, customer.phoneNumber);
        } catch (err) {
            console.error(err);
